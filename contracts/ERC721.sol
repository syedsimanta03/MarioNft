...
contract ERC721 {
  ...
  // Returns number of NFTS that a user own
  ...
  // Checks if can address is an operator for another address
  ...
  function getApproved(uint256 tokenId) public view returns(address) {
    require(_owners[tokenId] != address(0), 'Token ID does not match');
  ...
  // transferFrom but checks if onERC721Received is implemented when sending eth
